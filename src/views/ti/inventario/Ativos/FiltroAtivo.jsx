...
const schema = yup.object({
   ...
   ati_tipo_id: yup.array().of(yup.number()),
   ati_tipo_novo: yup.string(),
   ati_categoria_tipo: yup.array().of(yup.number()),
   ati_modelo_id: yup.number(),
   ati_modelo_novo: yup.string(),
   ati_fabricante_id: yup.array().of(yup.number()),
...
const FiltroAtivo = ({ open, handleDrawerOpen, filtro, handleFiltro, funcionarios, escritorios }) => {
   const { user } = useAuth();

   const { reset, control, setValue, handleSubmit, watch } = useForm({ defaultValues: { ati_status_id: 1 } });
