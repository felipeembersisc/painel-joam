...
const profileImage = import.meta.glob('@/assets/images/profile/*');

const keysAvatar = avatarImage.length > 0 ? Object.keys(avatarImage) : [];
const keysProfile = profileImage.length > 0 ? Object.keys(profileImage) : [];
...
const UserProfileCard = ({ avatar, name, profile, role, status }) => {
    ...
    const avatarProfile = avatar && keysAvatar.find(key => key.includes(avatar));
    const imageProfile = profile && keysProfile.find(key => key.includes(profile));
